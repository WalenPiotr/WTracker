...
class CutLayer extends React.Component<CutLayerProps, CutLayerState> {
    ...
    onDragEnd = (type: Timestamp) => (
        ...
        if (target instanceof HTMLCanvasElement) {
            const timestamp = evtObj.target.x() / dimensions.timeline.width;
    ...
    render() {
        const rect = {
            x: this.props.start * dimensions.timeline.width,
            y: (dimensions.canvas.height - dimensions.timeline.height) / 2,
            width:
                (this.props.end - this.props.start) * dimensions.timeline.width,
            height: dimensions.timeline.height,
        };
